...
class EventsNewActivityAdmin : AppCompatActivity() {
    ...
    private fun obtenerEventos() {
        Log.d("EventsNewActivityAdmin", "Iniciando la solicitud de eventos")

        RetrofitClient.instance.obtenerEventos().enqueue(object : Callback<List<Evento>> {
            override fun onResponse(call: Call<List<Evento>>, response: Response<List<Evento>>) {
                Log.d("EventsNewActivityAdmin", "Respuesta recibida: ${response.code()}")

                if (response.isSuccessful) {
                    val eventos = response.body() ?: emptyList()
                    Log.d("EventsNewActivityAdmin", "Cantidad de eventos recibidos: ${eventos.size}")
                    ...
                    Log.e("EventsNewActivityAdmin", "Error en la respuesta: ${response.errorBody()?.string()}")
                    Toast.makeText(this@EventsNewActivityAdmin, "Error al obtener eventos", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<List<Evento>>, t: Throwable) {
                Log.e("EventsNewActivityAdmin", "Error de red: ${t.message}", t)
